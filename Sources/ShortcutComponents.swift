...
struct ShortcutRoleSection: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(role.title)
                .font(FlowFonts.sectionTitle(14))
...
private struct ShortcutPresetRow: View {
    ...
    var body: some View {
        Button(action: action) {
            ...
            .padding(12)
            .background(isSelected ? FlowColors.accent.opacity(0.1) : FlowColors.surface)
            .cornerRadius(8)
            .overlay(
                RoundedRectangle(cornerRadius: 8)
                    .stroke(isSelected ? FlowColors.accent : FlowColors.border, lineWidth: 1.5)
            )
        }
        .buttonStyle(.plain)
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
                    ...
                    HStack(alignment: .center, spacing: 10) {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            Text(displayedBindingName)
                                .font(displayedBindingUsesMonospace ? .system(.body, design: .monospaced).weight(.semibold) : FlowFonts.body(13))
                                .foregroundStyle(FlowColors.textPrimary)
            ...
            if isCapturing {
                ...
                .font(FlowFonts.body(13).weight(.semibold))
