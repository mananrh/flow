...
import Carbon
import os.log
private let recordingLog = OSLog(subsystem: "com.mananrathod.flow", category: "Recording")
...
final class AppState: ObservableObject, @unchecked Sendable {
    ...
    private let contextPromptEnabledStorageKey = "context_prompt_enabled"
    private let contextPromptPermissionAskedKey = "context_prompt_permission_asked"
    ...
    @Published var contextPromptEnabled: Bool = false {
        didSet {
            UserDefaults.standard.set(contextPromptEnabled, forKey: contextPromptEnabledStorageKey)
        }
    }
    @Published var contextPromptPermissionAsked: Bool = false {
        didSet {
            UserDefaults.standard.set(contextPromptPermissionAsked, forKey: contextPromptPermissionAskedKey)
        }
    }
    ...
    init() {
        ...
        let contextPromptEnabled = UserDefaults.standard.bool(forKey: contextPromptEnabledStorageKey)
        let contextPromptPermissionAsked = UserDefaults.standard.bool(forKey: contextPromptPermissionAskedKey)
        ...
        self.contextPromptEnabled = contextPromptEnabled
        self.contextPromptPermissionAsked = contextPromptPermissionAsked
    ...
    /// URL of the flag file written while Flow is actively recording.
    ///
    ...
    ///
    /// Path: `~/Library/Application Support/Flow/is-recording`
    /// (or `Flow Dev/is-recording` when running the dev bundle).
    ...
    private static let recordingStateFlagQueue = DispatchQueue(
        label: "com.mananrathod.flow.recording-state-flag"
    )
    ...
    private func recordPipelineHistoryEntry(
        ...
        // Save to user-facing transcript history (only for non-empty transcripts)
        let trimmedClean = postProcessedTranscript.trimmingCharacters(in: .whitespacesAndNewlines)
        let trimmedRaw = rawTranscript.trimmingCharacters(in: .whitespacesAndNewlines)
        if !trimmedClean.isEmpty || !trimmedRaw.isEmpty {
            let transcriptRecord = TranscriptRecord(
                id: UUID(),
                timestamp: Date(),
                rawTranscript: trimmedRaw,
                cleanTranscript: trimmedClean,
            ...
            do {
                try transcriptHistoryStore.append(transcriptRecord)
            } catch {
                print("[TranscriptHistory] Failed to save transcript record: \(error.localizedDescription)")
            }
        }
    }
    ...
    private func startContextCapture() {
        // Skip context capture if disabled
        guard contextPromptEnabled else {
            lastContextSummary = "Context disabled"
            lastPostProcessingStatus = "Context disabled"
            capturedContext = nil
            return
        }
