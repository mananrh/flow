APP_NAME ?= Flow Dev
BUNDLE_ID ?= com.mananrathod.flow.dev
BUILD_DIR = build
APP_BUNDLE = $(BUILD_DIR)/$(APP_NAME).app
CODESIGN_IDENTITY ?= Flow Dev
...
# which Flow they are running when both are installed side by side.
ifeq ($(APP_NAME),Flow Dev)
...
ifeq ($(ARCH),universal)
	...
	@codesign --force --options runtime --sign "-" --entitlements Flow.entitlements "$(APP_BUNDLE)" 2>/dev/null || echo "Note: Code signing skipped (no identity)"
